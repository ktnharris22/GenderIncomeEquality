...
## 1b. Data Summarization
### Understanding the Basics
```{r, echo=FALSE}
#Jaih to clean up tables:
#- rename kable columns
#- titles for every table with brief blurb on value
kable(table(nlsy$sex))
kable(table(nlsy$race))
kable(table(nlsy$marital_status))
kable(table(nlsy$emp.industry))
kable(table(nlsy$birth.mom.age))
kable(table(nlsy$cocaine_use))
kable(table(nlsy$smokes))
kable(table(nlsy$drinks))
kable(table(nlsy$residence))
kable(table(nlsy$income.level_1997))
...
```
Here we can see both in the T-Test output as well as the bar chart that there is a statistically significant wage gap between genders. On average men make almost 10,000 dollars more than women.

## 2. Methodology
### What factors in particular play a role the gender wage gap?
We chose to explore several subsets of data first before running a linear model to see if we could identify any interesting patterns.

We'll start with race and marital status since those values intuitively seemed like they would have an impact.
...
Next we'll explore if substance use has any effect on the wage gap. 
...
### Age at Incarceration and HHIC_1997
```{r}
#Average net worth of household in 1997 by gender as reported by parent

#Data Clean to Remove Top Coding
# - top-coding for the purpose of this analysis does not provide meaningful
# information in understanding trends.  If anything it skewes the data in undescipherable ways
...
#No statistically significant difference in average houshold net worth between men and women
...
#### Does the gender wage gap persist over industries? Does it increase or decrease according to industry?
```{r, echo = FALSE}
...
# pairs(nlsy[,income.vars], lower.panel = panel.cor)
...
###The most significant out of the bunch
