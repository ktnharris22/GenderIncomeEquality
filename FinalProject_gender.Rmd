...
nlsy <- as_tibble(nlsy) %>%
  ...
  mutate(sex = recode_factor(sex, `1`="MALE", 
                             `2`="FEMALE")) %>%
  mutate(race = recode_factor(race, `1` = "BLACK", 
                              `2` = "HISPANIC", 
...
ggplot(data = income.data.HHIC1997, aes(x=sex, y=avg.HHIC_1997, fill=I("steelblue"))) + 
  ...
  guides(fill = FALSE) #No statistically signifiant difference in average houshold net worth between men and women
...
ggplot(data = gap.data.gender.race, aes(x=race, y=income.gap, fill=I("steelblue"))) + geom_bar(stat = "identity") +
  ...
  guides(fill = FALSE) +
    theme(axis.text.x=element_text(angle = -15, vjust = 0)) #Income gap does persist over race but there is noticeable difference in the gaps
...
ggplot(data = gap.data.marital_stat, aes(x=marital_status, y=income.gap, fill=I("steelblue"))) + geom_bar(stat = "identity") +
  ...
  guides(fill = FALSE) +
    theme(axis.text.x=element_text(angle = -15, vjust = 0)) #Huge gap in income between married men and women vs non-married
...
ggplot(data = gap.data.gender.smokes, aes(x=smokes, y=income.gap, fill=I("steelblue"))) + geom_bar(stat = "identity") +
  ...
  guides(fill = FALSE) #slight difference in wage gap between men and women who smoke and don't
...
ggplot(data = gap.data.gender.cocaine_use, aes(x=cocaine_use, y=income.gap, fill=I("steelblue"))) + geom_bar(stat = "identity") +
  ...
  guides(fill = FALSE) #Cocaine use does not seem to affect the wage gap at all. The bars are almost of equal height.
...
ggplot(data = gap.data.gender.drinks, aes(x=drinks, y=income.gap, fill=I("steelblue"))) + geom_bar(stat = "identity") +
  ...
  guides(fill = FALSE) #Noticeable difference in the wage gap between men and women who drink and don't drink
...
nlsy <- nlsy[nlsy$income > 0 & nlsy$income < 150000,] # only rows of people that make money within a certain range for income and housing. give jutification

nlsy <- nlsy[nlsy$income == 0, ] # further investigationon this population

gap.data.gender.industry <- nlsy %>% 
  group_by(`YEMP_INDCODE-2002.01_2017`, residence) %>%
  summarize(income.gap = mean(income[sex == "MALE"], na.rm = TRUE) -
              mean(income[sex == "FEMALE"], na.rm = TRUE),
...
ggplot(data = gap.data.gender.industry,aes(x=`YEMP_INDCODE-2002.01_2017`, y=income.gap, fill=I("steelblue"))) + 
  geom_bar(stat = "identity") +
  xlab("Industry") +
  ylab("Income Gap($)") +
...
#persistant wage gap between men and women even across genders


ggplot(
       subset(gap.data.gender.industry, residence %in% c("RURAL", "URBAN")),
       aes(x=`YEMP_INDCODE-2002.01_2017`, y=income.gap,fill=residence)) + 
  geom_bar(stat = "identity") +
...
```{r}
batch.one.var.names <-c("HHIC_1997", "residence", "race", "marital_status", "YEMP_INDCODE-2002.01_2017")
...
ggplot(data = gap.data.AGE_MOM_YOUTH, aes(x=CV_BIO_MOM_AGE_YOUTH_1997, y=income.gap, fill=is.significant)) + geom_bar(stat = "identity") +
