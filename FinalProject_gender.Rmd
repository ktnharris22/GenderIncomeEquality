...
lm.sex.industry <- lm(income ~ sex*emp.industry, data = nlsy)
...
Specifically marital status has the largest F-score at 299.29 with a p-value below .0001 suggesting that marital status is a highly significant factor when we discuss the gender wage gap. This is not surprising and follows what we saw earlier when we plotted the wage gap as a bar chart for different marital statuses.
```{r, echo = FALSE}
kable(coef(summary(lm.sex.mar_stat)), digits = c(0, 0, 2, 4))
```
