...
nlsy <- as_tibble(nlsy) %>%
  ...
  mutate(birth.mom.age = cut(nlsy$CV_BIO_MOM_AGE_YOUTH_1997, breaks = c(-Inf,0,19,29,39,49,59), labels = c("Unknown","Child/Teen","20s","30s","40s","50s+"),
                             ordered_result = TRUE)) %>%
...
lm.race.sex.mar_stat <- lm(income ~ sex*race*marital_status, data = nlsy) ###The most significant out of the bunch
